...
describe('CPU Engine Parity Tests', () => {
    let bus: Bus;
    ...
    function compareEngines(
        program: number[],
        cycles: number,
        verifyState?: (registers: CPURegisters) => void
    ) {
        if (!wasmEngine) {
            it.skip('WASM engine not available for comparison');
            return;
        }
        
        // Load program into memory
        for (let i = 0; i < program.length; i++) {
            bus.write(0x0000 + i, program[i]);
        }
    ...
    describe('State Transfer', () => {
        it('State save and load consistency', async () => {
            if (!wasmEngine) {
                it.skip('WASM engine not available');
                return;
            }
