...
describe('DebuggerLayout', () => {
    ...
    const setMemoryViewAddress = vi.fn((addr: number) => {
        memoryViewAddress = addr;
    });
    const setDisassemblerAddress = vi.fn((addr: number) => {
        disassemblerAddress = addr;
    });
    ...
    const renderWithProviders = (component: React.ReactNode) => {
        return render(
            <WorkerDataProvider workerManager={mockWorkerManager}>
                <DebuggerNavigationProvider>{component}</DebuggerNavigationProvider>
            </WorkerDataProvider>,
        );
    };
