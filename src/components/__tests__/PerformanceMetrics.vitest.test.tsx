...
describe('PerformanceMetrics', () => {
    ...
    beforeEach(() => {
        ...
        mockEngineStatus = {
            ...
            jsMetrics: {
                ...
                hostMillisPerSecond: 300, // 30% host load, ~3.3x headroom
            },
            wasmMetrics: {
                ...
                hostMillisPerSecond: 25, // 2.5% host load, ~40x headroom
            },
        };
    ...
    describe('Rendering', () => {
        ...
        it('should display current engine status', async () => {
            render(<PerformanceMetrics workerManager={mockWorkerManager as WorkerManager} updateInterval={999999} />);
        ...
        it('should show host CPU cost with headroom for the active engine', async () => {
            // JS active: hostMillisPerSecond=300 => 30.0% load, 3.3x headroom
            render(<PerformanceMetrics workerManager={mockWorkerManager as WorkerManager} updateInterval={999999} />);

            await waitFor(() => {
                expect(screen.getByText('Host CPU')).toBeInTheDocument();
                expect(screen.getByText(/30\.0%.*3\.3× headroom/)).toBeInTheDocument();
            });
        });
        ...
        it('should not render when engine status is null', async () => {
            ...
            const { container } = render(
                <PerformanceMetrics workerManager={mockWorkerManager as WorkerManager} updateInterval={999999} />,
            );
    ...
    describe('Error Handling', () => {
        it('should handle errors when fetching engine status', async () => {
            ...
            mockWorkerManager.getEngineStatus = vi.fn().mockRejectedValue(new Error('Failed to get status'));
