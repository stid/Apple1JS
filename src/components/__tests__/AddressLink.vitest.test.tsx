...
describe('AddressLink component', () => {
    ...
    describe('navigation', () => {
        ...
        it('should show correct title when context menu enabled', () => {
            ...
            expect(screen.getByRole('button')).toHaveAttribute('title', 'Click for menu • Ctrl/Cmd+Click to navigate');
        });

        it('should show correct title when context menu disabled', () => {
            render(<AddressLink address={0x1000} showContextMenu={false} />);

            expect(screen.getByRole('button')).toHaveAttribute('title', 'Ctrl/Cmd+Click to navigate');
        });
    });
    ...
    describe('event handling', () => {
        ...
        it('should have correct styles', () => {
            ...
            // `display` is a concrete value getComputedStyle resolves; the `inherit`
            // keywords are asserted via the inline style object since jsdom 29's
            // getComputedStyle no longer surfaces `inherit` (so toHaveStyle can't see it).
            expect(button).toHaveStyle({ display: 'inline' });
            expect(button.style.textAlign).toBe('inherit');
            expect(button.style.textDecoration).toBe('inherit');
            expect(button.style.lineHeight).toBe('inherit');
        });
    });
    ...
    describe('edge cases', () => {
        ...
        it('should not show run-to-cursor without worker', async () => {
            render(<AddressLink address={0x1234} showContextMenu={true} showRunToCursor={true} />);
