...
// One shared fetcher mock is reused across tests that instantiate
// useWorkerState<T> with different T, so its return must be assignable to
// every Promise<T> (Vitest 4 types vi.fn() strictly; v3 inferred `any`).
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- see above
type FetcherFn = (manager: WorkerManager) => Promise<any>;
type SubscriberFn = (manager: WorkerManager, callback: (data: unknown) => void) => Promise<(() => void) | void>;
...
describe('useWorkerState', () => {
    ...
    let mockFetcher: Mock<FetcherFn>;
    let mockSubscriber: Mock<SubscriberFn>;
    ...
    beforeEach(() => {
        ...
        mockFetcher = vi.fn<FetcherFn>().mockResolvedValue({ default: 'data' });
        mockSubscriber = vi.fn<SubscriberFn>();
    });
    ...
    it('should return initial value and loading state', async () => {
        ...
        const { result } = renderHook(() =>
            useWorkerState(mockWorkerManager, mockFetcher, undefined, { initialValue }),
        );
    ...
    it('should fetch data on mount', async () => {
        ...
        const { result } = renderHook(() => useWorkerState(mockWorkerManager, mockFetcher));
    ...
    it('should call custom error handler', async () => {
        ...
        renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, undefined, { onError }));
    ...
    it('should transform data', async () => {
        ...
        const { result } = renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, undefined, { transform }));
    ...
    it('should set up polling with interval', async () => {
        ...
        renderHook(() =>
            useWorkerState(mockWorkerManager, mockFetcher, undefined, { pollInterval: 1000, enablePolling: true }),
        );
    ...
    it('should set up subscription when provided', async () => {
        ...
        renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, mockSubscriber));
        ...
        expect(mockSubscriber).toHaveBeenCalledWith(mockWorkerManager, expect.any(Function));
    });
    ...
    it('should update data through subscription callback', async () => {
        ...
        const { result } = renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, mockSubscriber));
    ...
    it('should not update state after unmount', async () => {
        ...
        const { result, unmount } = renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, mockSubscriber));
    ...
    it('should handle subscription errors', async () => {
        ...
        renderHook(() => useWorkerState(mockWorkerManager, mockFetcher, mockSubscriber, { onError }));
    ...
    it('should re-subscribe when dependencies change', async () => {
        ...
        const { rerender } = renderHook(
            ({ dep }) => useWorkerState(mockWorkerManager, mockFetcher, mockSubscriber, { dependencies: [dep] }),
