...
> **Test-name form.** `none`-surface criteria carry the literal `AC-N:` with no surface suffix;
> only non-`none` surfaces get `AC-N (SURFACE):`. That is what the audit greps for.
...
- [x] **T1**: Write failing test named `AC-1: PIA powers up with all registers zero` in `src/core/__tests__/PIA6820-hw-accuracy.vitest.test.ts` — asserts both control, both direction and both output registers read zero after construction and after reset
- [x] **T2**: Write failing test named `AC-2: monitor port-B direction write reaches DDRB` in `src/core/__tests__/PIA6820-hw-accuracy.vitest.test.ts` — from power-on, writing `$7F` to the port-B address stores into DDRB and triggers no port-B output write
...
- [x] **T8**: Browser verification — boot the app, confirm the monitor prompt appears, keyboard input echoes, and no stray character precedes the prompt. Drive via in-page JS (the emulator loop never reaches `document_idle`)

### Tier 2 — Decimal mode in the Rust core (AC-6)

- [x] **T9**: Write failing test named `AC-6: decimal add and subtract follow BCD on both engines` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts` — asserts the TypeScript engine directly; the Rust half `skipIf`s in Node per the project's WASM test convention, and is covered by T11
- [x] **T10**: Modify `wasm-cpu/src/instructions_with_bus.rs` — add the decimal branch to `adc_bus` and `sbc_bus`, mirroring the TypeScript NMOS semantics (BCD result and carry; N/Z/V from the binary result)
- [x] **T11**: Run `yarn wasm:check`, then browser-verify decimal arithmetic on the Rust engine — switch engines at runtime and confirm the same BCD results as the TypeScript engine

### Tier 3 — TypeScript CPU correctness (AC-7…AC-9)

- [x] **T12**: Write failing test named `AC-7: indexed-indirect pointer wraps in zero page` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts`
- [x] **T13**: Write failing test named `AC-8: indirect jump reproduces the page-boundary vector bug` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts` — covers both the pointer dereference and the operand fetch
- [x] **T14**: Write failing test named `AC-9: software interrupt leaves the decimal flag alone` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts`
- [x] **T15**: Modify `src/core/cpu6502/addressing.ts` — wrap the indexed-indirect pointer high byte inside zero page; move the page-boundary bug off the operand fetch and onto the pointer dereference. Passes T12, T13
- [x] **T16**: Modify `src/core/cpu6502/instructions.ts` — stop clearing the decimal flag in the software-interrupt instruction. Passes T14

### Tier 4 — Store and read-modify-write cycle counts (AC-10)

- [x] **T17**: Write failing test named `AC-10: indexed stores and RMW cost a fixed cycle count` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts` — pins the published counts with and without a page crossing
- [x] **T18**: Modify `src/core/cpu6502/addressing.ts` — add the fixed-cost siblings `abxw`, `abyw`, `izyw`
- [x] **T19**: Modify `src/core/cpu6502/types.ts` — declare the three siblings on the CPU interface
- [x] **T20**: Modify `src/core/cpu6502/core.ts` — delegate the three siblings from the CPU class
- [x] **T21**: Modify `src/core/cpu6502/opcodes.ts` — repoint every writing opcode (stores, RMW, and the undocumented RMW forms) at the fixed-cost siblings. Passes T17

### Tier 5 — Terminal character set (AC-11)

- [x] **T22**: Write failing test named `AC-11 (RENDER): characters fold into the 64-glyph uppercase repertoire` in `src/apple1/__tests__/WebCRTVideo-hw-accuracy.vitest.test.ts` — lowercase folds to uppercase and no lowercase glyph is ever stored
- [x] **T23**: Modify `src/apple1/WebCRTVideo.ts` — fold incoming characters into the 64-glyph repertoire the way the original character generator does. Passes T22
- [x] **T24**: Browser verification — type lowercase at the prompt and confirm uppercase renders

### Tier 6 — Latent and cosmetic tail (AC-12…AC-17)

- [x] **T25**: Write failing test named `AC-12: reset preserves registers and decrements the stack pointer by three` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts`
- [x] **T26**: Modify `src/core/cpu6502/core.ts` — reset leaves accumulator, index registers and decimal flag untouched and decrements the stack pointer by three. Passes T25
- [x] **T27** [P]: Modify `wasm-cpu/src/cpu.rs` — same reset semantics in the Rust core
- [x] **T28**: Write failing test named `AC-13: non-maskable interrupt latches on the assertion edge` in `src/core/__tests__/CPU6502-hw-accuracy.vitest.test.ts`
...
- [x] **T36**: Modify `src/core/types/` (bus space type) — add the optional offset mask field
- [x] **T37**: Modify `src/core/Bus.ts` — apply the mask when computing the decoded offset
- [x] **T38**: Modify `src/apple1/index.ts` — widen the PIA mapping to the mirrored region and set its mask. Passes T35
- [x] **T39** [P]: Modify `wasm-cpu/src/bus.rs` — widen the IO region to match, so the Rust engine forwards mirrored addresses
...
- [x] **T51** [P]: Modify `src/core/RAM.ts` — record the cleared-on-power-on deviation per D-012 in a comment
- [x] **T52**: Modify `docs/active/hardware-accuracy-audit.md` — annotate each finding as closed, with the AC that covers it; keep the deviations recorded as intentional (power-on memory, control-line output path, backspace) marked as such
