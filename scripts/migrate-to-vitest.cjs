...
function migrateTestFile(filePath) {
    ...
    content = content.replace(/import.*from ['"]@jest\/globals['"];\s*\n?/g, '');
    
    // Remove other Jest imports
    content = content.replace(/import.*from ['"]@testing-library\/jest-dom\/extend-expect['"];\s*\n?/g, '');
    ...
    if (content.includes('jest.fn()') || content.includes('jest.mock') || content.includes('jest.spyOn')) imports.push('vi');
    
    // Add vitest import if needed and not already present
    if (imports.length > 0 && !content.includes("from 'vitest'")) {
